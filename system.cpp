...
void inverse(float** matrix, int n){
    ...
    // cholesky_decomp(matrix, n);

    // transform_lower(matrix, cpy, n);

    // mult_mat(cpy, matrix,n);

    // forward_elim(matrix, 3);
...
Particle add_new_landmark(Particle particle, float *z, float (&Q_mat)[2][2]){
    ...
    gz = new float*[2]; 
    gv_inv = new float*[2]; 
    gv_trans = new float*[2];

    for (int i = 0; i < 2; i++){
        gz[i] = new float[2]; 
        gv_inv[i] = new float[2]; 
        gv_trans[i] = new float[2];
        Q[i] = new float[2];
    }
    ...
    // invert_mat(gv_trans);
    inverse(gv_trans, 2);
    mult_mat(Q, gv_trans,2);
    mult_mat(gv_inv, gv_trans,2);
